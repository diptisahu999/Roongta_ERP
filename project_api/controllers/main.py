...
class ProjectApiController(http.Controller):
        ...
        Body (JSON):
            {
                ...
                "department_id": <int>,          (required, department ID or department_name)
                "department_name": "str",        (required, department name or department_id)
                "assigned_user_ids": [<int>],    (required, assigned users IDs or assigned_user_names)
                "assigned_user_names": ["str"],  (required, assigned users names or assigned_user_ids)
                "tag_name": "Urgent",            (required, single tag name or tag_id)
                "tag_id": <int>                  (required, single tag ID or tag_name)
            }
        """
        try:
            uid = _authenticate_api()
            ...
            # 2. Required: Department (department_id or department_name)
            if 'department_id' in body and body['department_id']:
                dept_id = int(body['department_id'])
                dept = request.env['hr.department'].with_user(uid).browse(dept_id)
                if not dept.exists():
                    return _error(f"Department with id={dept_id} not found.", status=404)
                ...
                return _error("'department_id' or 'department_name' field is required.")

            # 3. Required: Assigned User(s) (assigned_user_ids or assigned_user_names)
            if 'assigned_user_ids' in body and isinstance(body['assigned_user_ids'], list) and body['assigned_user_ids']:
                ...
                if not names_list:
                    return _error("'assigned_user_names' field is required and cannot be empty.")
                ...
                return _error("'assigned_user_ids' or 'assigned_user_names' field is required.")

            # 4. Required: Tag (tag_name or tag_id)
            ...
            if not selected_tag_id:
                return _error("'tag_name' (or 'tag_id') field is required when creating a project.")
    ...
    def create_task(self, **kwargs):
        ...
        Body (JSON):
            {
                ...
                "tag_name": "Urgent",            (required - single tag string or tag_id)
                "description": "...",            (optional)
        ...
        try:
            ...
            if not selected_tag_id:
                return _error("'tag_name' (or 'tag_id') field is required when creating a task.")

            vals = {
                'name': name,
                'project_id': int(project_id),
                'tag_ids': [(6, 0, [selected_tag_id])],
            }
